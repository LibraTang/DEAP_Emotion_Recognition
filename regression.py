...
Val_R = RandomForestRegressor(n_estimators=512, n_jobs=-1)
Val_R.fit(data_train[0:data_train.shape[0]:32], Valence_Train[0:data_train.shape[0]:32])
joblib.dump(Val_R, "model/val_model.pkl")


Aro_R = RandomForestRegressor(n_estimators=512, n_jobs=-1)
Aro_R.fit(data_train[0:data_train.shape[0]:32], Arousal_Train[0:data_train.shape[0]:32])
joblib.dump(Aro_R, "model/aro_model.pkl")


Dom_R = RandomForestRegressor(n_estimators=512, n_jobs=-1)
Dom_R.fit(data_train[0:data_train.shape[0]:32], Domain_Train[0:data_train.shape[0]:32])
joblib.dump(Dom_R, "model/dom_model.pkl")


Lik_R = RandomForestRegressor(n_estimators=512, n_jobs=-1)
Lik_R.fit(data_train[0:data_train.shape[0]:32], Like_Train[0:data_train.shape[0]:32])
joblib.dump(Lik_R, "model/lik_model.pkl")
